...
# Solving B: The monthly incentives you will provide on each policy to maximize the net revenue 
	...
	# Preparing df
		...
		df_test = head(df_test,5000) # temporal

	# Building incentives_p
		# First rule: P <= 1
		# Second rule: If P = 1 then Incentive = 0
		# Third rule: max improvement is ~0.1729
		# Third rule bis: below P = 0.8271 no incentive 'll make P=1

		# What is the max Improvement they can reach?
		df_test$maxP = 1- df_test$renewal_est
		...
		TotalNetRevenue = function(incentives_p){

			#delta_p = sapply(incentives_p,function(y) IncToImprov(y))/100
			delta_p = sapply(incentives_p,function(y) 20*(1-exp(-(10*(1-exp(-y/400)))/5)))/100

			return(-sum((to_optimize$renewal_est + delta_p)*to_optimize$premium-incentives_p))
		}

		Mapfun <- function(x){ round(x,digits=0)}
start_time <- Sys.time()
		optm_par=  DEoptim(fn=TotalNetRevenue, lower=lowers, upper=uppers,fnMap = Mapfun,
			control=list(NP=10*nrow(to_optimize), itermax=50, parallelType=1, trace = FALSE,parVar=c("to_optimize")))
end_time <- Sys.time()

		#optm = DEoptim(fn=TotalNetRevenue,lower=lowers,upper=uppers,DEoptim.control(itermax=50, trace = FALSE))

		incentive_results = optm_par$optim$bestmem # devuelve mejor vector
		names(incentive_results) = NULL

		TNR = -optm_par$optim$bestval # devuelve mejor valor
